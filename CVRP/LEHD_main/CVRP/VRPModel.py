...
class VRPModel(nn.Module):
    ...
    def forward(self, state, selected_node_list, solution, current_step,Batch_capacity=None,):
        # solution's shape : [B, V]
        self.capacity = Batch_capacity[:,[0]]
...
class CVRP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        for i in range(self.head_num):
            self.heads.append(nn.ModuleList([DecoderLayer(**model_params) for _ in range(head_layer_num)]))
        self.heads = nn.ModuleList(self.heads)

        self.Linear_finals =nn.ModuleList([nn.Linear(embedding_dim, 2, bias=True) for _ in range(self.head_num)])

        interval = 500 - 10

        self.capacitysss = [0,int(interval*1/3),int(interval*2/3)]
    ...
    def forward(self, data,selected_node_list,capacity,remaining_capacity):
        ...
        remaining_capacity = remaining_capacity.reshape(batch_size_V,1,1)/capacity.unsqueeze(2)
        ...
        props = torch.zeros(batch_size_V, 2 * (customer_num))

        for jjj in range(self.head_num):
            if jjj==self.head_num-1:
                index1 = self.capacitysss[jjj]<capacity.squeeze(1)
            else:
                index1 =  (self.capacitysss[jjj]<capacity.squeeze(1)) * (capacity.squeeze(1)<=self.capacitysss[jjj+1])
            ...
            if index2.any():
                ...
                out1 = self.Linear_finals[jjj](out1)  # shape: [B*(V-1), reminding_nodes_number + 2, embedding_dim ]
                out1[:, [0, -1], :] = out1[:, [0, -1], :] + float('-inf')  # first node、last node
                out1 = torch.cat((out1[:, :, 0], out1[:, :, 1]), dim=1)  # shape:(B, 2 * ( V - current_step ))
                props1 = F.softmax(out1, dim=-1)

                props1 = torch.cat((props1[:, 1:customer_num + 1], props1[:, customer_num + 1 + 1 + 1:-1]),
                                  dim=1)
                props[index2]=props1
