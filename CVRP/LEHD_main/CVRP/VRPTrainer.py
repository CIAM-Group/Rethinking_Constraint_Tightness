...
from CVRP.LEHD_main.CVRP.VRPModel import VRPModel as Model
from CVRP.LEHD_main.CVRP.validate import main_validate
from CVRP.LEHD_main.CVRP.VRPEnv import VRPEnv as Env
from CVRP.LEHD_main.utils.utils import *
...
class VRPTrainer:
    ...
    def run(self):
        ...
        raw_data_demand = None
        raw_data_cost = None
        raw_data_node_flag = None

        for data_path in self.env_params['data_path']:

            if raw_data_nodes is None:

                raw_data_nodes, raw_data_capacity, raw_data_demand, raw_data_cost, raw_data_node_flag =  \
                self.env.load_raw_data(data_path=data_path[0], episode=data_path[1] )

            else:

                raw_data_nodes_tmp, raw_data_capacity_tmp, raw_data_demand_tmp, raw_data_cost_tmp, \
                raw_data_node_flag_tmp = \
                    self.env.load_raw_data(data_path=data_path[0], episode=data_path[1])

                raw_data_nodes = torch.cat((raw_data_nodes, raw_data_nodes_tmp),dim=0)
                raw_data_capacity = torch.cat((raw_data_capacity, raw_data_capacity_tmp),dim=0)
                raw_data_demand = torch.cat((raw_data_demand, raw_data_demand_tmp),dim=0)
                raw_data_cost = torch.cat((raw_data_cost, raw_data_cost_tmp), dim=0)
                raw_data_node_flag = torch.cat((raw_data_node_flag, raw_data_node_flag_tmp), dim=0)

        self.env.raw_data_nodes = raw_data_nodes
        self.env.raw_data_capacity = raw_data_capacity
        self.env.raw_data_demand = raw_data_demand
        self.env.raw_data_cost = raw_data_cost
        self.env.raw_data_node_flag = raw_data_node_flag

        print(self.env.raw_data_nodes.shape, 'C_min',torch.min(self.env.raw_data_capacity), 'C_max',torch.max(self.env.raw_data_capacity))
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            if all_done or (epoch % model_save_interval) == 0:
                ...
                current_gaps = []
                for validate_data in self.trainer_params['validate_data']:
                    score_optimal, score_student ,gap = main_validate(epoch,self.result_folder, use_RRC = False,
                                                                  cuda_device_num=self.trainer_params['cuda_device_num'],
                                                                  validate_data=validate_data[0])
                    current_gaps.append([ "C"+str(validate_data[1]), np.round(score_optimal,6),np.round(score_student,6),np.round(gap,6),'-------'])

                current_gaps = np.array(current_gaps).ravel().tolist()
                save_gap.append([epoch,*current_gaps])
            ...
            if current_step ==0:
                ...
                loss_node, selected_teacher, selected_student, selected_flag_teacher, selected_flag_student = \
                    self.model(state, self.env.selected_node_list, self.env.solution, current_step,
                               Batch_capacity=self.env.Batch_capacity)
