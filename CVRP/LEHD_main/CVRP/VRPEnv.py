...
class VRPEnv:
    ...
    def load_problems(self, episode, batch_size, ):
        ...
        if self.env_params['mode']=='train':
            ...
            index = (self.raw_data_capacity<=500).long()

            index2 = index.gt(0)
            current_raw_data_nodes = self.raw_data_nodes[index2]
            current_raw_data_demand = self.raw_data_demand[index2]
            current_raw_data_capacity = self.raw_data_capacity[index2]
            current_raw_data_node_flag = self.raw_data_node_flag[index2]

            index3 = torch.randperm(len(current_raw_data_nodes)).long()
            current_raw_data_nodes = current_raw_data_nodes[index3]
            current_raw_data_demand = current_raw_data_demand[index3]
            current_raw_data_capacity = current_raw_data_capacity[index3]
            current_raw_data_node_flag = current_raw_data_node_flag[index3]

            self.problems_nodes = current_raw_data_nodes[:batch_size]
            self.Batch_demand = current_raw_data_demand[:batch_size]
            self.Batch_capacity = current_raw_data_capacity[:batch_size]
            self.solution = current_raw_data_node_flag[:batch_size]
    ...
    def sampling_subpaths(self, problems, solution, length_fix=False):
        ...
        double_solution_demand = problems[:offset_index, :, 2][
            torch.arange(offset_index)[:, None].repeat(1, double_solution.shape[1]), double_solution[:offset_index, :,
                                                                                     0]]
        ...
        index_2, _ = torch.cat(
            (new_sulution_ascending, new_sulution_ascending, new_sulution_ascending, new_sulution_ascending), dim=1). \
    ...
    def load_raw_data(self, data_path = None, episode=1000000):
        ...
        if self.env_params['mode'] == 'train':
            ...
            for line in tqdm(open(data_path, "r").readlines()[0:int(0.5 * episode)], ascii=True):
                line = line.split(",")
            ...
            for line in tqdm(open(data_path, "r").readlines()[int(0.5 * episode):int(episode)], ascii=True):
                line = line.split(",")
                depot_index = int(line.index('depot'))
                customer_index = int(line.index('customer'))
                capacity_index = int(line.index('capacity'))
    ...
    def step(self, selected, selected_student, selected_flag_teacher, selected_flag_student):
        ...
        # current_capacities = self.Batch_capacity[is_depot,0].unsqueeze(1).float().clone()

        # print('self.problems[is_depot, :, 3] \n',self.problems[is_depot, :, 3].shape)
        #
        # print('self.Batch_capacity \n',self.Batch_capacity.shape)
        #
        # print( self.problems[is_depot, :, 3].dtype)
        #
        # print( self.Batch_capacity[is_depot,0].unsqueeze(1).dtype)

        self.problems[is_depot, :, 3] = self.Batch_capacity[is_depot,0].unsqueeze(1).float().clone()

        # print(self.Batch_capacity[is_depot,0].unsqueeze(1).shape)
        #
        # assert False
        ...
        self.problems[smaller_, :, 3] = self.Batch_capacity[smaller_,0].unsqueeze(1).float().clone()
    ...
    def sampling_subpaths_repair(self, problems, solution, length_fix=False, mode='test', repair=True):
        ...
        sub_solution = double_solution[sub_solu_index1, sub_solu_index2, sub_solu_index3].reshape(batch_size,
                                                                                                  length_of_subpath, 2)
