...
test_paras = {
   # problem_size: [filename, episode, batch]
    100: [ 'vrp100_validation_hgs_n1000_C10_validation.txt',1000,1000],
}
...
def main_validate(epoch,path,use_RRC=None,cuda_device_num=None,validate_data=None):
    if DEBUG_MODE:
        _set_debug_mode()
    ...
    if validate_data is not None:
        env_params['data_path']= b + f"/data/validation/{validate_data}"
    tester = Tester(env_params=env_params,
                    model_params=model_params,
                    tester_params=tester_params)
    if cuda_device_num is None:
