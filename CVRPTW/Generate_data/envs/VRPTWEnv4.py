...
import sys
sys.setrecursionlimit(20000)  # 将递归深度限制增加到 2000
@dataclass
class Reset_State:
...
class VRPTWEnv4:
    ...
    def generate_dataset(self, num_samples, problem_size, path, window=0.2, speed=1.0, scale_tw=1.0):
        data = self.get_random_problems(num_samples, problem_size, normalized=False, window=window, speed=speed, scale_tw=scale_tw)
    ...
    def get_random_problems(self, batch_size, problem_size, normalized=True, window=0.2, speed=1.0,scale_tw=1.0):
        ...
        service_time = torch.ones(batch_size, problem_size) * window
        ...
        # tw_start = tw_start/scale_tw
        # tw_end = tw_end * scale_tw
        # self.speed = self.speed*scale_tw
        # service_time = service_time/scale_tw
        # self.depot_end = self.depot_end*scale_tw

        ###############################################################################
        ###############################################################################
        ###############################################################################

        # node level scale factor
        name = f'hgs_vrptw100_uniform_0.2_n1000_speed_1.0_scale_{scale_tw}.pt'
        scale_tw = torch.empty(batch_size, problem_size).uniform_(0.01, 3.0)  # shape: [4,100]

        torch.save(scale_tw, name)

        # scale the start and end time
        interval = tw_end - tw_start  # shape: [4,100]
        interval_2 = (interval / 2) * (1 - scale_tw)  # shape: [4,100]
        tw_start = tw_start + interval_2  # shape: [4,100]
        tw_start = torch.clamp(tw_start, min=0.0)  # shape: [4,100]
        tw_end = tw_end - interval_2  # shape: [4,100]
        # print(tw_start)
        # print(tw_end)
        # scale the service time, only allow to relax the service time
        gt_index = scale_tw > 1.0  # shape: [4,100]

        # print(service_time.shape, gt_index.shape,  scale_tw.shape, scale_tw[gt_index].shape, service_time[gt_index].shape)
        service_time[gt_index] = service_time[gt_index] / scale_tw[gt_index]  # shape: [4,100]
        ...
        # window = window/scale_tw
        # self.depot_end = self.depot_end * scale_tw
        # if self.depot_end < 3+window:
        #     self.depot_end = 3+window
        ...
        # self.depot_end = torch.clamp(self.depot_end, min=2.82843)
        # distance1 = (depot_xy - node_xy).norm(p=2, dim=-1)
        # min =
        ...
        #     # tw_start = tw_start / scale_tw
        #
        #     interval = tw_end - tw_start
        #     interval_2 = (interval/2)*(1-scale_tw)
        #     tw_start = tw_start + interval_2
        #     tw_end = tw_end - interval_2
        #
        #
        #
        # else:
        ...
        #     tw_end = tw_end * scale_tw
        #     tw_start = tw_start / scale_tw
        # # print('self.depot_end',self.depot_end)
        ...
        # print(torch.max(0 + (depot_xy - node_xy).norm(p=2, dim=-1) / self.speed,
        #                 tw_start))
        # print(service_time)
        # print(torch.max(0 + (depot_xy - node_xy).norm(p=2, dim=-1) / self.speed,
        #                        tw_start) + service_time + (node_xy - depot_xy).norm(p=2, dim=-1) / self.speed )
        # print()
        total_time = torch.max(0 + (depot_xy - node_xy).norm(p=2, dim=-1) / self.speed,
                               tw_start) + service_time + (node_xy - depot_xy).norm(p=2, dim=-1) / self.speed > self.depot_end + round_error_epsilon
        # (batch, problem)


        # print(total_time.ravel())
        instance_invalid = total_time.any()

        if instance_invalid:
            print(">> Invalid instances, Re-generating ...")
            # assert False
            return self.get_random_problems(batch_size, problem_size, normalized=normalized, window=window, speed=speed, scale_tw=scale_tw)
        elif normalized:
