...
class Trainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs']+1):
            ...
            if epoch == 1 or (epoch % validation_interval == 0):
                ...
                paths = ['vrptw100_uniform_0.2_n1000_speed_1.0_scale_1.0.pkl']
                val_envs = [get_env(prob)[0] for prob in val_problems]
                for i, path in enumerate(paths):
                    # if no optimal solution provided, set compute_gap to False
                    score, gap = self._val_and_stat(dir[i], path, val_envs[i](**{"problem_size": problem_size, "pomo_size": problem_size}),
                                                    batch_size=500, val_episodes=val_episodes, compute_gap=True)
    ...
    def _train_one_epoch(self, epoch):
        ...
        while episode < train_num_episode:
            # print(episode,'/',train_num_episode)
            ...
            data, scale_tw = env.get_random_problems_2(batch_size, self.env_params["problem_size"])
            avg_score, avg_loss = self._train_one_batch(data, env, scale_tw)
    ...
    def _train_one_batch(self, data, env, scale_tw):
        # print('scale_tw',scale_tw)
        self.model.train()
        ...
        self.model.pre_forward(reset_state,scale_tw)
        ...
        while not done:
            selected, prob = self.model(state, scale_tw=scale_tw)
            # shape: (batch, pomo)
    ...
    def _val_one_batch(self, data, env, aug_factor=1, eval_type="argmax", scale_tw=1.0):
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            state, reward, done = env.pre_step()
            while not done:
                selected, _ = self.model(state,scale_tw=scale_tw)
                # shape: (batch, pomo)
                state, reward, done = env.step(selected)
    ...
    def _val_and_stat(self, dir, val_path, env, batch_size=500, val_episodes=1000, compute_gap=False):
        ...
        # 使用正则表达式提取 scale_ 和 .pkl 之间的数值
        match = re.search(r'scale_([\d.]+)\.pkl', val_path)


        scale_tw = float(match.group(1))
        ...
        while episode < val_episodes:
            ...
            scale_tw = torch.ones(size=(bs,1))*scale_tw

            data = env.load_dataset(os.path.join(dir, val_path), offset=episode, num_samples=bs)
            no_aug, aug = self._val_one_batch(data, env, aug_factor=1, eval_type="argmax", scale_tw=scale_tw)
