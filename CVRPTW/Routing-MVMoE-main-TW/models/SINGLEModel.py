...
class SINGLEModel(nn.Module):
    ...
    def pre_forward(self, reset_state, scale_tw):
        ...
        self.encoded_nodes = self.encoder(depot_xy, node_xy_demand_tw, scale_tw)
    ...
    def forward(self, state, selected=None, scale_tw=None):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, attr, ninf_mask=state.ninf_mask, scale_tw=scale_tw)
...
class SINGLE_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        for i in range(self.head_num):
            self.heads.append(nn.ModuleList([EncoderLayer(**model_params) for _ in range(head_layer_num)]))
        self.heads = nn.ModuleList(self.heads)

        self.capacitysss = [0,1.0,2.0]

    def forward(self, depot_xy, node_xy_demand_tw, scale_tw):
        ...
        out_all = torch.zeros(size=out.shape)

        for jjj in range(self.head_num):
            if jjj == self.head_num - 1:
                index1 = self.capacitysss[jjj] < scale_tw.squeeze(1)
            else:
                index1 = (self.capacitysss[jjj] < scale_tw.squeeze(1)) * (
                            scale_tw.squeeze(1) <= self.capacitysss[jjj + 1])

            index2 = index1.long().gt(0.5)
            if index2.any():

                out1 = out[index2].clone()

                current_head = self.heads[jjj]
                for layer in current_head:
                    out1 = layer(out1)

                out_all[index2] = out1
                # out1 = self.heads[jjj](out1)
...
class SINGLE_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        if self.problem in ["CVRP", "VRPB"]:
            ...
            # self.Wq_last2 = nn.Linear(embedding_dim + 2, head_num * qkv_dim, bias=False)
            # self.Wq_last3 = nn.Linear(embedding_dim + 2, head_num * qkv_dim, bias=False)
        ...
        # self.single_head_key2 = None  # saved, for single-head attention
        # self.single_head_key3 = None  # saved, for single-head attention

        # self.W_s1 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        # self.W_s2 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        # self.W_s3 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        ...
        # self.k2 = None  # saved key, for multi-head attention
        # self.v2 = None  # saved value, for multi-head_attention
        #
        # self.Wk3 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        # self.Wv3 = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        # self.multi_head_combine3 = nn.Linear(head_num * qkv_dim, embedding_dim)
        # self.k3 = None  # saved key, for multi-head attention
        # self.v3 = None  # saved value, for multi-head_attention
        #
        # self.feedForward1 = FeedForward(**model_params)
        # self.feedForward2 = FeedForward(**model_params)
        # self.feedForward3 = FeedForward(**model_params)

        self.capacitysss = [1.0, 2.0]
    ...
    def set_kv(self, encoded_nodes):
        ...
        # encoded_nodes1 = self.W_s1(encoded_nodes)
        self.single_head_key = encoded_nodes.transpose(1, 2)
        # shape: (batch, embedding, problem+1)


        # self.k2 = reshape_by_heads(self.Wk2(encoded_nodes), head_num=head_num)
        # self.v2 = reshape_by_heads(self.Wv2(encoded_nodes), head_num=head_num)
        # encoded_nodes2 = self.W_s2(encoded_nodes)
        # self.single_head_key2 = encoded_nodes2.transpose(1, 2)
        #
        # self.k3 = reshape_by_heads(self.Wk3(encoded_nodes), head_num=head_num)
        # self.v3 = reshape_by_heads(self.Wv3(encoded_nodes), head_num=head_num)
        #
        # encoded_nodes3 = self.W_s3(encoded_nodes)
        # self.single_head_key3 = encoded_nodes3.transpose(1, 2)
    ...
    def forward(self, encoded_last_node, attr, ninf_mask, scale_tw=None):
        ...
        # props = torch.zeros(size=ninf_mask.shape)
        # mh_atten_out_all = torch.zeros(size=encoded_last_node.shape)
        ...
        probs1 = F.softmax(score_masked, dim=2)
        # shape: (batch, pomo, problem)
        props = probs1
        return props
