...
class VRPTWEnv:
    ...
    def get_random_problems_2(self, batch_size, problem_size, normalized=True):
        depot_xy = torch.rand(size=(batch_size, 1, 2))  # (batch, 1, 2)
        node_xy = torch.rand(size=(batch_size, problem_size, 2))  # (batch, problem, 2)
        ...
        ######################################################################################
        ################ Scale factor (corresponding to \alpha in our paper) is used to adjust the tightness of the time window.
        ######################################################################################
        ...
        if instance_invalid:
            print(">> Invalid instances, Re-generating ...")
            # assert False
            return self.get_random_problems_2(batch_size, problem_size, normalized=normalized)
        elif normalized:
            node_demand = torch.randint(1, 10, size=(batch_size, problem_size)) / float(demand_scaler)  # (batch, problem)
            return (depot_xy, node_xy, node_demand, service_time, tw_start, tw_end), scale_tw
        else:
            node_demand = torch.Tensor(np.random.randint(1, 10, size=(batch_size, problem_size)))  # (unnormalized) shape: (batch, problem)
            capacity = torch.Tensor(np.full(batch_size, demand_scaler))
            return (depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end), scale_tw
