...
class Tester:
    ...
    def _test(self, model, env_class, compute_gap=False):
        ...
        while episode < test_num_episode:
            ...
            match = re.search(r'scale_([\d.]+)\.pkl', data_path)

            scale_tw = float(match.group(1))
            scale_tw = torch.ones(size=(batch_size, 1)) * scale_tw
            score, aug_score, all_score, all_aug_score = self._test_one_batch(model, data, env, scale_tw)
            ...
            if all_done:
                ...
                #
                if compute_gap:
                    print(f'teacher_length:', np.mean(np.array(opt_sols)))
                    print(f'gap:', (score_AM.avg - np.mean(np.array(opt_sols)))/np.mean(np.array(opt_sols)))
        return scores, aug_scores

    def _test_one_batch(self, model, test_data, env, scale_tw):
        ...
        with torch.no_grad():
            ...
            model.pre_forward(reset_state,scale_tw)

        # POMO Rollout
        state, reward, done = env.pre_step()
        while not done:
            selected, _ = model(state, scale_tw=scale_tw)
            # shape: (batch, pomo)
